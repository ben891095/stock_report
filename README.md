...
每個交易日收盤後產生一頁台股收盤報表，以部落格形式收錄的靜態網站。純 HTML／CSS，JavaScript、沒有依賴、沒有建置流程。
...
prompts/daily-tw-stock-report.md  # 排程流程（Prompt）
...
| 檔案 | 負責 |
|---|---|
| `prompts/daily-tw-stock-report.md` | 流程：讀規則 → 取資料 → 產頁 → 更新首頁 → 提交 GitHub |
| `.agents/skills/.../SKILL.md` | 內容：Yahoo 擷取欄位、驗證計算、指標判讀、章節規格 |
| `templates/report.html` | 結構：標籤、class、章節順序的唯一依據 |

每次執行會**新增一頁報表並修改 `index.html` 加上連結**，兩者提交在同一分支（`main`）。資料不完整、尚未收盤或休市時，流程會回報 `SKIPPED` 並且不寫入任何檔案。

## 手動新增一篇報表

1. 複製 `templates/report.html` 為 `reports/YYYY/MM/YYYY-MM-DD.html`。
2. 取代 `{{DATE}}`、`{{WEEKDAY}}`、`{{TREND}}`（盤勢偏向）、`{{TREND_CLASS}}`（`is-up`／`is-down`／`is-flat`），並填入五個章節。
3. 編輯 `index.html`：
...
- **紅漲綠跌**：沿用台股慣例，與歐美市場相反。
- **判讀 tag**：大盤指標每列標偏多／偏空／中性，只代表該項指標，不是結論；總結的「盤勢偏向」另依均線、動能、量價、法人、類股五個面向判定。
- **BIAS**：標過熱／中性／超跌，不參與方向判定。
